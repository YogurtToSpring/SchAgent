...
function applyStreamEvent(eventName, data, result, handlers) {
  ...
  if (eventName === "tool_result") {
    const toolName = data.name || data.tool || "unknown";
    let call = [...result.tool_calls]
      .reverse()
      .find((item) => item.tool === toolName && item.output == null);
    if (!call) {
      call = {
        tool: toolName,
        label: toolLabel(toolName),
...
function normalizeChatResponse(response) {
  const normalized = {
    ...
    files: Array.isArray(response.files)
      ? response.files.map(normalizeFile).filter((file) => file.name)
      : [],
  };
...
function mergeFiles(current = [], next = []) {
  ...
  for (const file of next) {
    ...
    const exists = merged.some(
      (item) => item.name === file.name || item.path === file.path,
    );
...
function artifactKey(artifact = {}) {
  if (artifact.type === "file")
    return `file:${artifact.name || artifact.path || artifact.url}`;
...
function splitReasoning(text) {
  ...
  if (
    reasoningMarker &&
    answerMarker &&
    reasoningMarker.index < answerMarker.index
  ) {
    return {
      reasoning: normalized
        .slice(reasoningMarker.index, answerMarker.index)
        .replace(/^(思考过程|思考|推理过程|Reasoning)[:：]/i, "")
        .trim(),
      answer: normalized
        .slice(answerMarker.index)
        .replace(/^(正式回复|最终回答|答案)[:：]/, "")
        .trim(),
    };
  }
...
async function streamMockResponse(payload, handlers) {
  ...
  const reasoning =
    "根据当前用户角色、课表数据和问题意图，判断需要查询课程与天气信息。";
...
async function mockChatResponse({
  ...
  if (needsDate(text)) {
    return {
      ...
      answer:
        "我还需要知道你想查询哪一天。可以直接回复“今天”“明天”或具体日期。",
      missing_fields: ["date"],
  ...
  if (
    role === "teacher" &&
    (text.includes("班") || text.includes("冲突") || text.includes("提醒"))
  ) {
...
function teacherClassResponse(sessionId, platformContext = {}) {
  const targetClass = platformContext.classes?.find((item) =>
    item.name.includes("软件工程1班"),
  ) ||
    platformContext.classes?.[0] || {
      id: "class-se-1",
      name: "软件工程1班",
    };
  const courses = (platformContext.courses || []).filter(
    (item) => item.classId === targetClass.id,
  );

  return {
    session_id: sessionId,
    status: "success",
    answer: `已按教师权限查询 **${targetClass.name}** 的课表。当前未发现明显时间冲突；如果需要，我可以继续生成班级课程提醒。`,
    steps: [
      "识别任务：班级课表查询 / 冲突检查",
      "校验当前用户角色：教师管理员",
      "调用班级查询工具",
    ...
    tool_calls: [
      ...
      },
      {
        ...
        output: {
          ...
          courses: courses.map(
            (item) =>
              `${item.weekday} ${item.startTime}-${item.endTime} ${item.courseName}`,
          ),
        },
      },
      {
...
function needsDate(text) {
  const asksSchedule =
    text.includes("有没有课") || text.includes("课表") || text.includes("课程");
  const hasDate =
    text.includes("今天") ||
    text.includes("明天") ||
    text.includes("后天") ||
